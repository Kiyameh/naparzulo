...
export const login = async (req, res) => {
  ...
  try {
    ...
    if (!isMatch) {
      return res
        .status(400)
        .json({message: '<Moogonse> Email o contraseña incorrectos'})
    }
...
// Función para verificar si el navegador tiene un token válido almacenado
export const verifyToken = async (req, res) => {
  ...
  jwt.verify(token, process.env.LOGIN_TOKEN_SECRET, async (err, user) => {
    if (err)
      return res.status(401).json({message: '<Moogonse> Token no valido'})

    const userFound = await User.findById(user.id)
    if (!userFound)
      return res.status(401).json({message: '<Moogonse> Usuario no valido'})
